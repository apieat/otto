		...
		err = vm.Set("ext2", func(c FunctionCall) Value {
			{
				...
				is(ctx.Stacktrace, []string{
					"github.com/apieat/otto.TestNativeStackFrames.func1.2 (native_stack_test.go:29)",
					"B (input.js:3:19)",
					"github.com/apieat/otto.TestNativeStackFrames.func1.1 (native_stack_test.go:20)",
					"A (input.js:2:19)", "input.js:4:4",
				})

				is(ctx.Callee, "github.com/apieat/otto.TestNativeStackFrames.func1.2")
		...
		err = vm.Set("ext3", func(c FunctionCall) Value {
			{
				...
				is(ctx.Stacktrace, []string{
					"github.com/apieat/otto.TestNativeStackFrames.func1.3 (native_stack_test.go:71)",
					"github.com/apieat/otto.TestNativeStackFrames.func1.2 (native_stack_test.go:29)",
					"B (input.js:3:19)",
					"github.com/apieat/otto.TestNativeStackFrames.func1.1 (native_stack_test.go:20)",
					"A (input.js:2:19)", "input.js:4:4",
				...
				is(ctx.Callee, "github.com/apieat/otto.TestNativeStackFrames.func1.3")
				is(ctx.Filename, "native_stack_test.go")
				is(ctx.Line, 71)
				is(ctx.Column, 0)
			}
